...
def main():
    ...
    for i, (train_idx, val_idx) in enumerate(kf.split(indices)):
        ...
        val_loader = torch.utils.data.DataLoader(
                        dataset, 
                        batch_size= args.batch_size, pin_memory = False, sampler=val_sampler)
        # TEST SET
        if i == 0:
        ...
        logger = Logger(os.path.join(args.experiment_path, args.experiment_name,f'logs_{i}.log'))
    ...
    torch.save(best_model, os.path.join(args.experiment_path, args.experiment_name,'model.pt'))
...
def getMeanScores(scores_dict, k=5):
    mean = [0, 0,0,0]
    keys = ['pre','rec','f1','iou']
