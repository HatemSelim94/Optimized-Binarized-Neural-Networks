...
def main():
    ...
    val_dataset = DataSets.get_dataset(args.data_name, no_of_classes=args.num_of_classes, transforms=val_transforms)
    ...
    for i,(cv_train_idx, cv_val_idx) in enumerate(kf.split(indices)):
        print('---------------------------')
        print(f'Fold:{i}')
        print(len(cv_val_idx))
        print(cv_val_idx)
        print('---------------------------')
        ...
        if args.k > 0:
            ...
            sub_train_sampler = SubsetRandomSampler(cv_train_idx,generator=train_generator)
            sub_val_sampler = SubsetRandomSampler(cv_val_idx, generator=val_generator)
        ...
        train_loader = torch.utils.data.DataLoader(
                        train_dataset, 
                        batch_size=args.batch_size, pin_memory = False, sampler=sub_train_sampler)
        val_loader = torch.utils.data.DataLoader(
                        val_dataset, 
                        batch_size= args.batch_size, pin_memory = False,sampler=sub_val_sampler)
        if i ==0:
        ...
        logger = Logger(os.path.join(args.experiment_path, args.experiment_name,f'logs.log'))
        ...
        if args.train:
            ...
            for epoch in range(args.epochs):
                ...
                if miou>best_miou:
                    ...
                    best_epoch = epoch
                    best_scores = {'cls':miou,'cat':cat_miou}
                print(f'Fold: {i}')
            ...
            mean_scores.append(best_scores)
            ...
            torch.save(best_model, os.path.join(args.experiment_path, args.experiment_name,f'model_fold_{i}.pt'))
    print(getMeanScores(mean_scores,args.k))
...
def getMeanScores(scores_list, k=5):
    mean = [0, 0]
    keys = ['cls','cat']
    ...
    return {key:s/k for s,key in zip(mean,keys)} 
