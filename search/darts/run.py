import sys
sys.path.append("/home/hatem/Projects/server/final_repo/src")
import torch
...
from utilities import infer, set_seeds, Clipper, DataPlotter, Tracker, train_arch
...
parser.add_argument('--validation_subset', type=int, default=200)
...
parser.add_argument('--init_channels', type=int, default=20)
parser.add_argument('--network_optim', type=str, default='adamax')
parser.add_argument('--network_optim_bin_lr', type=str, default='0.01')
parser.add_argument('--network_optim_bn_lr', type=str, default='0.001')
...
def main():
    ...
    train_transforms = Transformer.get_transforms({'normalize':{'mean':CityScapes.mean,'std':CityScapes.std}, 'resize':{'size':args.image_size},'random_horizontal_flip':{'flip_prob':0.2}})
    val_transforms = Transformer.get_transforms({'normalize':{'mean':CityScapes.mean,'std':CityScapes.std},'resize':{'size':[320,320]}})
    ...
    val_idx = range(args.validation_subset) 
    ...
    #logging.info(f'param size = {size_in_mb:0.4f}MB')
    # SGD or Adam is used to optimize model parameters w not α alpha
    optim_args = [[{'params':fp_params, 'weight_decay':args.weight_decay,'lr':1e-5},{'params':bin_params, 'weight_decay':0,'lr':1e-3}]]
    optim_kwargs = {'betas':args.betas}
    optimizer = Clipper.get_clipped_optim('Adamax', optim_args, optim_kwargs)
