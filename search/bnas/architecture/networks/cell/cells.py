...
class NCell(nn.Module):
    ...
    def forward(self, input0, input1,  skip_input, idx):
        ...
        for i in range(self.node_num):
            s = self.sum([self.edges[offset+j](h, F.softmax(self.alphas[offset+j], dim=-1), idx[offset+j]) for j, h in enumerate(states[-2:])]) #offset +j = 2
            offset += 2
            states.append(s)
