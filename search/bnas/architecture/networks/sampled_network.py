...
class SampledNetwork(nn.Module):
    def __init__(self, args, bnas_idx):
        super(SampledNetwork, self).__init__()
        self.bnas_idx = bnas_idx
        self.args = args
        ...
        for i, cell_type in enumerate(self.cells_sequence): 
            if cell_type == 'r':
                ...
                continue
            if cell_type =='r':
                cell_specs=(c, c_prev_prev, c_prev, prev_cell, self.alphas[idx[cell_type]] ,self.initial_channels, 2, self.ops_num,self.nodes_num, self.binary, self.affine,self.padding_mode, self.jit, self.dropout2d,self.binarization, self.activation, True)
            elif cell_type == 'n':
                cell_specs=(c, c_prev_prev, c_prev, prev_cell, self.alphas[idx[cell_type]],1,2,self.ops_num, self.nodes_num, self.binary, self.affine,self.padding_mode, self.jit,self.dropout2d, self.binarization, self.activation, True)
            else:
                cell_specs=(c, c_prev_prev, c_prev, prev_cell, self.alphas[idx[cell_type]], 2, self.ops_num,self.nodes_num,self.binary, self.affine, self.padding_mode,self.jit,self.dropout2d, self.binarization, self.activation, True)
