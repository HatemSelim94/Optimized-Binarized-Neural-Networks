...
from utilities import infer, set_seeds, Clipper, DataPlotter, Tracker, train_arch, model_info, clean_dir, prepare_ops_metrics, train
...
parser.add_argument('--init_epochs', type=int, default=40)
...
parser.add_argument('--experiment_path', type=str, default='search/bnas/experiments/')
...
parser.add_argument('--t', type=int, default=3, help='number of samples')
...
def main():
    ...
    # optimitzation
    ops_length = net.set_worst_primitives()
    K = net.get_k() 
    while(K!=1):
        for T in range(args.t):
            for _ in range(ops_length):
                sample_network = net.sample_network()
                sample_fp_params = [p for p in net.parameters() if not hasattr(p,'bin')]
                sample_bin_params = [p for p in net.parameters() if hasattr(p,'bin')]
                sample_optim_args = [[{'params':sample_fp_params, 'weight_decay':args.network_optim_fp_weight_decay,'lr':args.network_optim_fp_lr},{'params':sample_bin_params, 'weight_decay':0,'lr':args.network_optim_bin_lr, 'betas':[args.network_optim_bin_betas, args.network_optim_bin_betas ]}]]
                sample_network_optimizer = Clipper.get_clipped_optim(args.network_optim, sample_optim_args)
                for e in range(5): # few epochs
                    train(train_loader, val_loader, sample_network, None, criterion, sample_network_optimizer, lr,e,2)
                val_iou, _ = infer(val_loader, sample_network, criterion)
                net.update_score(t=T, score=val_iou)
        net.reudce_space()
        K = net.get_k()
        print(K)
        for ep in range(4):
            train_arch(train_loader, val_loader, arch, criterion,optimizer,ep, arch_start=2)
    gene = net.genotype()
    print(gene)
