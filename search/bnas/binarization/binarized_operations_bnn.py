...
from .binarization import BnnActivation, BnnConv2d, BnnTConv2d, bnn_hubara_binarization
bn_eps = 1e-5
bn_norm_momentum = 0.1
...
class BNNActivation(nn.Module):
  def __init__(self):
      ...
      self.activation = BnnActivation(bnn_hubara_binarization)
  def forward(self, x):
...
class SingleConv2d(nn.Module):
  def __init__(self, C_in, C_out, kernel_size, stride, padding=0 ,step=None):
      ...
      self._op = BnnConv2d(C_in, C_out, kernel_size, stride, padding, quantization_class=bnn_hubara_binarization, step=step)
  def forward(self, x):
    return self._op(x)

class BnnSum(nn.Module):
  def __init__(self):
      super(BnnSum,self).__init__()
      self.binarize = Activation()
...
class BnnConv(nn.Module):
  def __init__(self, C_in, C_out, kernel_size, stride, padding=0 ,step=None,affine=True):
      ...
      self.op = nn.Sequential(
        ...
        nn.BatchNorm2d(C_out, affine=affine,eps=bn_eps,momentum = bn_norm_momentum),
        nn.Tanh(),
